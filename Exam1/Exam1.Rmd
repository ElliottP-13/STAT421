...
a. Let $X$ be the random variable denoting how many of the vials are ineffective. So $S_X = \{0, 1, 2, 3\}$. Since we can assume that they do not come from the same batch, we assume that all the vials are independent. So we use a Binomial distribution to model this $Y \sim Binomial(3, 0.2)$ since we have 3 samples so $n = 3$ and the probability that the vial is ineffective is 20% = 0.2 (this is our event). Then 
